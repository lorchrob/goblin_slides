...
\begin{enumerate}
  \item Existing approaches to input generation
  % \item \system{} philosophy
  \item Syntax and language features
  \item Semantics 
  \item \system{} workflow
  \begin{itemize}
    \item Well-formedness checks
    \item Derived fields
    \item Main search algorithm
  \end{itemize}
...
\begin{frame}
\begin{enumerate}
  \item \green{\textbf{Existing approaches to input generation}}
  % \item \system{} philosophy
  \item Syntax and language features
  \item Semantics 
  \item \system{} workflow
...
\begin{frame} 
    \frametitle{Wishlist} 
    An input generation approach that gives...\pause
    \begin{enumerate}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        ...
        \item Supporting constraint solving over \green{arbitrary SMT theories} \pause
        ...
        \item What is a context-sensitive grammar? See ANTLR, yacc \pause
\begin{lstlisting}[style=goblin]
<xml-tree> ::= <openclose-tag> | 
               <open-tag> <inner-tree> <close-tag>
...
\begin{frame}
    ...
    \system{} is an \green{input generator}, but not a (complete) \green{fuzzer} \\[8pt]\pause
        ...
        {\scriptsize\begin{tikzpicture}[
            ...
            \node[box][opacity=.3] (dr) at (4,1) {Fuzzing driver};
            \node[box][opacity=.3] (sut) at (6,1) {SUT};
            \node[data][opacity=.3] (sutout) at (6,0) {SUT output};

            % Arrows
            \draw[->] (inp.east) -- (sys.west);
            \draw[->] (sys.east) -- (sutinp.west);
            \draw[->][opacity=.3] (sutinp.south) -- (dr.north);
            \draw[->][opacity=.3] (dr.east) -- (sut.west);
            \draw[->][opacity=.3] (dr.west) -| (sys.south);
            \draw[->][opacity=.3] (sut.south) -- (sutout.north);
            \draw[->][opacity=.3] (sutout.west) -| (dr.south);
        \end{tikzpicture}}}
\end{frame}
...
\begin{frame}[noframenumbering]
    \frametitle{Input generation vs fuzzing} 
    Most tools perform mutations at the \green{byte level} \\[8pt]
        \resizebox{\linewidth}{!}{
        {\scriptsize\begin{tikzpicture}[
            box/.style={draw, rectangle, minimum width=1cm, minimum height=.5cm, align=center},
            data/.style={draw, trapezium, trapezium left angle=70, trapezium right angle=110,
                 minimum width=1.2cm, minimum height=.5cm, align=center},
            dataemph/.style={draw, trapezium, trapezium left angle=70, trapezium right angle=110,
                 minimum width=1.2cm, minimum height=.5cm, align=center, fill=lightgray}
            ]
 
            % Nodes with manual coordinates
            \node[data] (inp) at (0,2) {Grammar/spec};
            \node[box] (sys) at (2,2) {\system{}};
            \node[dataemph] (sutinp) at (4,2) {SUT input};
            \node[box] (dr) at (4,1) {Fuzzing driver};
            \node[box] (sut) at (6,1) {SUT};
            \node[data] (sutout) at (6,0) {SUT output};
...
\begin{frame}[noframenumbering]
    \frametitle{Input generation vs fuzzing} 
    Some tools perform \green{structure-aware mutations} (ATFuzz, SAECRED) \\[8pt]
        \resizebox{\linewidth}{!}{
        {\scriptsize\begin{tikzpicture}[
            box/.style={draw, rectangle, minimum width=1cm, minimum height=.5cm, align=center},
            data/.style={draw, trapezium, trapezium left angle=70, trapezium right angle=110,
            ...
            % Nodes with manual coordinates
            \node[dataemph] (inp) at (0,2) {Grammar/spec};
            \node[box] (sys) at (2,2) {\system{}};
            \node[data] (sutinp) at (4,2) {SUT input};
            \node[box] (dr) at (4,1) {Fuzzing driver};
            \node[box] (sut) at (6,1) {SUT};
...
\begin{frame}[fragile]
    \frametitle{Structure-aware mutations}
How to perform \green{context-sensitive, structure-aware} mutations? \pause

\green{Crossover mutation} swapping {\color{teal}\scriptsize\texttt{GROUP\_ID}} and {\color{teal}\scriptsize\texttt{RG\_LIST}}
\begin{lstlisting}[style=plain]
COMMIT ::= SEQ @\color{teal}GROUP\_ID@ SCALAR 
  { @\color{teal}GROUP\_ID@ is equal to 13  and  SEQ is greater than 4 }
RG_CONT ::= RG_LENGTH RG_TY @\color{teal}RG\_LIST@
  { RG_LENGTH is the length of @\color{teal}RG\_LIST@ }
...
-->
COMMIT ::= SEQ @\color{teal}RG\_LIST@ SCALAR
  { SEQ is greater than 4 }
RG_CONT ::= RG_LENGTH RG_TY @\color{teal}GROUP\_ID@
  { @\color{teal}GROUP\_ID@ is equal to 13 }
...
\end{lstlisting}\pause 
\vspace{-4pt}
\center {{\textbf{Prior work cannot tie \green{global constraints} to mutations}}}
...
\hline
\textbf{Tool} & \textbf{Constraint type} & \textbf{Philosophy} &
\textbf{Search algo} & \textbf{Supported theories} \\
\hline
\system{} & Local & Minimalist & Constraint solving & All \\
ISLa & Global & Maximalist & Constraint solving & Strings \\
Fandango & Global & Maximalist & Genetic algo & All \\
\hline
\end{tabular}
\end{table}

\center{Fandango constraints may be \green{non-monotonic} (no monotonically decreasing notion of distance for constraint satisfaction)
and time out for simple examples (equality, set membership)}
\end{frame}

% \begin{frame}\frametitle{Prior work} \pause
% ISLa
% \begin{itemize}
%   \item Global constraints make structure-aware mutations less precise
%   \item Only natively supports string constraints
%   \item ``Maximalist'' design philosophy
% \end{itemize} \pause
...
\begin{enumerate}
  \item Existing approaches to input generation
  % \item \system{} philosophy
  \item \green{\textbf{Syntax and language features}}
  \item Semantics 
  \item \system{} workflow
  \begin{itemize}
    \item Well-formedness checks
...
\begin{frame}\frametitle{\system{} by example: Language features}
  \system{}'s \green{minimalist} design philosophy positions it as an \green{intermediate language} 
  (compile target), not a \green{user-facing language} (future work)
\end{frame}

\begin{frame}[fragile]
    \frametitle{Language features: Production rules}\pause
\system{} grammars have \green{production rules} as in CFGs\\[4pt]\pause
\begin{lstlisting}[style=goblin]
...
\begin{frame}[fragile]
    \frametitle{Language features: Type annotations}\pause
...
\begin{frame}[fragile]
    \frametitle{Language features: Refinement types}\pause
...
\begin{frame}[fragile]
    \frametitle{Language features: Semantic constraints}\pause
Attach \green{semantic constraints} to production rules
...
\begin{frame}[fragile]
    \frametitle{Language features: Dot notation}\pause
...
\begin{frame}[fragile]
    \frametitle{Language features: Derived fields}\pause
...
\begin{frame}[fragile]
    \frametitle{Language features: Derived fields}
\begin{lstlisting}[style=goblin]
@\color{darkred}<PACKET>@ ::= @\color{teal}<TYPE>@ @\color{teal}<AUX>@ @\color{darkred}<PAYLOAD>@
@\highlight@  { @\color{teal}<AUX>@ <- @\color{darkred}<PAYLOAD>@.@\color{teal}<F1>@ bvmul @\color{darkred}<PAYLOAD>@.@\color{teal}<F2>@;
...
...
Operators \blue{\scriptsize\texttt{<-}} and \blue{\scriptsize\texttt{=}} are semantically interchangeable\pause 

But try telling an SMT solver to find \blue{$x$} such that \blue{$\mathsf{hash}(x) = y$}...
\end{frame}
...
% \begin{itemize}
  What are the semantics of a \system{} grammar? \pause
...
\newcommand{\dttree}{%
    \begin{tikzpicture}[
      ...
      \node {1: $\langle$Packet$\rangle[0]$} [-to]
        child[opacity=.3] { node {2: $\langle$Type$\rangle[0]$}
          child { node[leaf] {5: $0x01$} }}
        child { node {3: $\langle$Aux$\rangle[0]$}
          child { node[leaf] {6: $0x06$} } }
        child { node {4: $\langle$Payload$\rangle[0]$} 
          child { node {7: $\langle$F1$\rangle[0]$} child { node[leaf] {10: $0x02$} } }
          child { node {8: $\langle$F2$\rangle[0]$} child { node[leaf] {11: $0x03$} } }
          child[opacity=.3] { node {9: $\langle$Bytes$\rangle[0]$} child { node {12: $\langle$Byte$\rangle[1]$} child { node[leaf] {13: $0x21$} } } } };
    \end{tikzpicture}%
  }
  {\center \dttree}
\end{frame}
...
\newcommand{\dttree}{%
    \begin{tikzpicture}[
      ...
      \node[opacity=.3] {1: $\langle$Packet$\rangle[0]$} [-to]
        child[opacity=.3] { node {2: $\langle$Type$\rangle[0]$}
          child { node[leaf] {5: $0x01$} }}
        child[opacity=.3] { node {3: $\langle$Aux$\rangle[0]$}
          child { node[leaf] {6: $0x06$} } }
        child[opacity=.3] { node {4: $\langle$Payload$\rangle[0]$} 
          child { node {7: $\langle$F1$\rangle[0]$} child { node[leaf] {10: $0x02$} } }
          child { node {8: $\langle$F2$\rangle[0]$} child { node[leaf] {11: $0x03$} } }
...
\begin{enumerate}
  ...
  \item Syntax and language features
  \item Semantics
  \item \green{\textbf{\system{} workflow}}
  \begin{itemize}
    \item \green{\textbf{Well-formedness checks}}
    \item \green{\textbf{Derived fields}}
    \item \green{\textbf{Main search algorithm}}
  \end{itemize}
  \item Formal guarantees
  \item Evaluation
  \item Discussion and future work
...
\begin{frame}
    ...
    \begin{itemize}
        \item[1.] Disallow \green{cyclic dependencies} (e.g. \blue{$\langle A \rangle \leftarrow T[\langle B\rangle], \langle B\rangle \leftarrow T[\langle A\rangle]$})\pause
        % \begin{itemize}
        %     \item[$(i)$] Build a directed graph for each production rule\pause
        %     \item[$(ii)$] Include vertex for each RHS nonterminal/symbolic terminal\pause
        %     \item[$(iii)$] Include an edge from $\langle A \rangle$ to $\langle B \rangle$ when  
        %     $\langle A \rangle$'s definition includes $\langle B \rangle$ as a subterm
        %     \item[$(iv)$] Check for cycles\pause
        % \end{itemize}
        \item[2.] Disallow derived fields in semantic constraints\pause
        \begin{itemize}
            \item Two stages of computation
...
We remove derived fields from \blue{$G$} before the main search; 
...
Start with \green{context-free case} \pause

A \green{derivation tree} is an AST that may be \green{open} (or \green{closed})\pause
\begin{enumerate}
\item Build candidate \blue{$\mathit{dt}$} via random walk until closed (as in XML)
...
\begin{frame}[fragile]
    \frametitle{Search Algorithm: Main Concepts}
Some ``decisions'' are \green{forced} (see solid arrows): symbolic terminals and 
...
\begin{frame}[fragile]
    \frametitle{Search Algorithm: Main Concepts}
\green{Termination} is not guaranteed... \pause 

\center {\textbf{Idea: Bound search depth with IDS}}
...
\begin{frame} \frametitle{Search Algorithm: Main Concepts}
    ...
    \vspace{-4pt}
    \center {\textbf{Idea: utilize incrementality of SMT engine to repair flawed} 
    \textbf{candidate solutions (fear of commitment)}} \pause 
    \begin{enumerate}
      ...
      \item Do not commit to concrete values; call {\scriptsize\blue{\texttt{get-model}}} once \blue{$dt$} is closed; then instantiate
    \end{enumerate}
\end{frame}
...
\newcommand{\dttreee}{%
  \begin{tikzpicture}[
    ...
    \node {$\langle$Packet$\rangle[0]$} [-to]
      child[opacity=.3] { node {$\langle$Type$\rangle[0]$}
        child { node {sym\_leaf} }}
      child[opacity=.3] { node {$\langle$Aux$\rangle[0]$}
        child { node {dep\_leaf} } }
      child { node {$\langle$Payload$\rangle[0]$} 
        child[opacity=.3] { node[yshift=-18pt] {$\langle$F1$\rangle[0]$} 
          child { node {sym\_leaf} } }
        child[opacity=.3] { node[yshift=-18pt] {$\langle$F2$\rangle[0]$} 
          child { node {sym\_leaf} }}
        child { node[yshift=-18pt] {$\langle$Bytes$\rangle[0]$} 
          % shift children of Bytes down to elongate arrows
          child[-to,dotted] { node[yshift=-18pt] {$\langle$Byte$\rangle[0]$}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item Main search algorithm
  \end{itemize}
  \item \green{\textbf{Formal guarantees}}
  \item Evaluation
  \item Discussion and future work
\end{enumerate}
\end{frame}
...
\begin{enumerate}
  ...
  \item Formal guarantees
  \item Evaluation
  \item \green{\textbf{Discussion and future}}
\end{enumerate}
\end{frame}
