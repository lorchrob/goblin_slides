...
\newcommand{\dttree}{%
  ...
  \green{Not} a string {\scriptsize \texttt{0x0106020321}}
...
\begin{frame} 
    ...
    How does \system{} find \blue{$t$} such that \blue{$t \in \mathcal{L}_{\textsf{AST}}(G)$}?\pause
    \begin{enumerate}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        \item \green{Context-free language emptiness} is a modeling issue, stemming from 
        \green{non-wellfounded recursion} \pause
        \item We perform a more general check called \green{context-free wellformedness}\pause
        \item[1.]  Iteratively expand a set \blue{$S$} of nonterminals known to be able to produce finite derivations 
        until reaching a fixpoint\pause
        \item[2.] Check every reachable nonterminal is in \blue{$S$}\pause
    \end{itemize}
\begin{lstlisting}[style=goblin]
...
\begin{frame}
    ...
    Derived fields {\scriptsize \texttt{<F> <- expr}} must be computable \green{without constraint solving}
    \begin{itemize}
        \item[1.] Disallow \green{cyclic dependencies}
        \item[2.] Disallow derived fields in semantic constraints
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        in \green{k-path coverage} for $k=3$ (percentage of paths of length $3$ traversed through the grammar)\pause
        \item Outperform prior work by $\sim$$10-100\%$ in all metrics, save for efficiency of CSV inputs
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    Case study 4\pause
    \begin{itemize}
        \item SAECRED input generation (SyGuS engine; \system{} predecessor) \pause
        \item Handle bit vector SMT constraints not expressible in ISLa \pause
        \item 36x improvement in efficiency \pause
        \item Able to produce outputs for more than twice the number of grammars 
        ($\sim$24,000 / $\sim$97000 up to $\sim$49,000 / $\sim$97,000)
    \end{itemize}
...
\begin{frame}
    ...
    \begin{itemize}
        \item User-facing language (attribute grammars, built-ins like {\scriptsize\texttt{length(.)}})\pause
        \item More powerful type system (e.g., polymorphism)\pause
        \item AI abstraction/refinement algorithms\pause
        \item CDCL-style backjumping
    \end{itemize}
    % e.g., CLP semantics
\end{frame}
